...
def auto_init(func):
    ...
    def wrapper(self, *args, **kwargs):
        arg_names = func.__code__.co_varnames[1:func.__code__.co_argcount] # 获取 func 的参数名列表，除了第一个参数（通常是 self）
        for name, arg in zip(arg_names, args):
            setattr(self, name, arg) # setattr 将每个参数值设置为实例的属性
        for name, arg in kwargs.items():
            setattr(self, name, arg)
        func(self, *args, **kwargs) # 调用func函数(即类的 __init__ 方法),确保 __init__ 方法中定义的额外逻辑都会被执行
