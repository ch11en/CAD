...
if [ -z "${model_name_or_path}" ]; then
    model_name_or_path="/ssd-data1/cxf2022/00.model_base/t5_large"
fi
...
CUDA_VISIBLE_DEVICES=${CUDA_IDS} python train_quad.py \
  --devices=8 \
  ...
  --mode "train_test" \
  --self_training_data_dir "${self_training_data_dir}" \
