...
import sys, os
sys.path.append('/data/cxf2022/dl_project/01.ST/code')
...
class DataModule(pl.LightningDataModule):
    ...
    def load_labeled_dataset(self):
        ...
        self.raw_datasets_with_target_sent = self.add_target_data_to_raw_dataset(self.raw_datasets)  # 含以下键值对['sentence', 'quads', 'ID', 'target_sentence']
        self.new_datasets = self.add_tokenized_input_to_raw_dataset(self.raw_datasets_with_target_sent)  # [ {'sentence', 'quads', 'ID', 'target_sentence', 'tokenized_sent_input_ids', 'tokenized_sent_attention_mask', 'tokenized_target_sent'} ]
    ...
    def generate_target_sentence(self, quad):
        ...
        if ot == 'NULL':
            ot = 'none'
        quad_seq = ' | '.join([at, ot, ac, sp])  # after : [aspect | opinion | category | sentiment]   ;   original : [category | sentiment | aspect | opinion]
    ...
    def add_self_training_data(self):
        ...
        if setting == 'full':  # 如果 setting 是 full，则不进行过滤
            ...
            try:
                ...
                self_training_data = sorted(self_training_data, key=lambda e: e['reward'][0], reverse=True)[int(start): int(end)]  # 例如，setting 为 10-90 时，表示截取 reward 最高的前 10% 到 90% 的数据。
    ...
    def load_unlabeled_dataset(self, max_example_num=1_000_000):
        ...
        def process_and_filter(raw_sentence):
            ...
            new_sentence = re.sub(r'\((?P<v1>[^ ])(?P<v2>.*)(?P<v3>[^ ])\)', lambda x: '( ' + x.group('v1') + x.group('v2') + x.group('v3') + ' )', raw_sentence) # 匹配括号内的内容，要求括号内的第一个和最后一个字符是非空格字符。在括号内的内容前后添加空格，使其格式变为 ( content )。
        ...
        self.new_datasets = {'predict': predict_examples}  # original : self.raw_datasets = {'predict': predict_examples}
...
class DataCollator:
    ...
    def __call__(self, examples):
        ...
        if self.mode in ('train', 'dev', 'test'):
            target_seqs, target_ids, target_mask = self.make_labels(examples)  # target_seqs : [category | sentiment | aspect | opinion]
        ...
        if self.mode != 'predict':
            contrastive_labels['at'] = self.get_at_labels(all_quads)  # example : self.contrastive_labels: {'at': [1, 1, 1, 1, 1, 1, 1, 1], 'ot': [1, 1, 1, 1, 1, 1, 1, 1], 'sp': [2, 2, 2, 2, 2, 2, 2, 2]}
    ...
    def make_labels(self, examples):
        target_seqs = [self.make_quads_seq(example) for example in examples]  # [category | sentiment | aspect | opinion]
    ...
    def get_grid_labels(self, inputs_in, targets_in, target_seqs, truncated=False):
        ...
        grid_member_dict = {'pad': -100, 'none': 0, 'aspect': 1, 'opinion': 2, 'negative': 3, 'neutral': 4, 'positive': 5, }
        grid_matrixes = []
        mask_matrixes = []
        ...
        for input_idx in range(len(inputs_in)):
            ...
            for i in range(1, non_pad_target_len + 1):
                ...
                target_dense_str = tokenizer.decode(targets_encoded[:i], ignore=-100 ,clean_up_tokenization_spaces=False).replace(' ', '')
                ...
                for i in range(len(at_dense)):
                    ...
                    if at_dense != 'none':
                        if at_dense[i] in sentence_dense_str:
                            ...
                            at_dense_left, at_dense_right = sentence_dense_to_decoded[at_dense_left], sentence_dense_to_decoded[at_dense_right] + 1
                            ...
                            grid_matrix[at_dense_left: at_dense_right, at_dense_left: at_dense_right] = grid_member_dict['aspect']  # at_dense在句子中的位子置1
                        # else:
                ...
                for i in range(len(ot_dense)):
                    ...
                    if ot_dense != 'none':
                        if ot_dense[i] in sentence_dense_str:
                            ...
                            op_dense_left, op_dense_right = sentence_dense_to_decoded[op_dense_left], sentence_dense_to_decoded[op_dense_right] + 1
                            # grid_matrix[asc_left: asc_right, :non_pad_sentence_len] = grid_member_dict['none']
                            # grid_matrix[asc_left: asc_right, op_left: op_right] = grid_member_dict['opinion']
                            grid_matrix[op_dense_left: op_dense_right, op_dense_left: op_dense_right] = grid_member_dict['opinion']  # op在句子中的位子置2
                        # else:
...
class LightningModule(pl.LightningModule):
    ...
    def forward(self, **inputs):
        # print(inputs.keys(), '\n')  # dict_keys(['input_ids', 'attention_mask', 'labels', 'target_mask', 'examples', 'ac_grid_label', 'ac_grid_mask'])

        examples = inputs.pop('examples')
        if not self.args.use_sl:
            targets = inputs.pop('target_mask')
            ac_grid_label = inputs.pop('ac_grid_label')
            ac_grid_mask = inputs.pop('ac_grid_mask')
    ...
    def on_validation_epoch_end(self):
        ...
        self.validation_step_outputs.clear()  # 清空列表
    ...
    def on_test_epoch_end(self):
        ...
        self.test_step_outputs.clear()  # 清空列表
...
class LightningModule_SL(pl.LightningModule):
    ...
    def _step(self, batch):
        ...
        outputs, encoder_last_state, decoder_hidden_states, sp_pred, ot_pred, at_pred, pooled_encoder_layer = self.forward( input_ids=batch["input_ids"],
                                                                                                                            attention_mask=batch["attention_mask"],
                                                                                                                            labels=tar_labels)

        criterion = SupConLoss(loss_scaling_factor=self.args.cont_loss, temperature=self.args.cont_temp)  # define loss with a temperature `temp`
        ...
        if self.ac_loss_lambda > 0:
            ...
            loss += (ac_loss(avg_lm_logits.view(-1, avg_lm_logits.size(-1)), tar_labels.view(-1)) * self.args.ac_loss_lambda)
    ...
    #         print(f"warning! don't find dataset path: {dataset_path}")
    #         return
...
class Element_learner(nn.Module):
    ...
    def __init__(self, model_name_or_path=None):
        super().__init__()
        self.in_features = 1024 # if model_name_or_path == 't5_base' else 1024
...
class Grid_learner(nn.Module):
    ...
    def __init__(self, args):
        super().__init__()
        self.args = args
        self.in_features = 1024 # if self.args.model_name_or_path == 't5_base' else 1024
    ...
    def forward(self, encoder_last_hidden_state, decoder_hidden_states, encoder_mask, decoder_mask, grid_labels):
        ...
        grid_labels = torch.stack(grid_labels)
        # print('encoder_last_hidden_state.size() :', encoder_last_hidden_state.size())   # encoder_last_hidden_state.size()  : torch.Size([8, 44, 1024])
        # print('decoder_hidden_states.size() :', decoder_hidden_states.size())           # decoder_hidden_states.size()      : torch.Size([8, 53, 1024])
        # print('encoder_mask.size() :', encoder_mask.size())                             # encoder_mask.size()               : torch.Size([8, 44])
        # print('decoder_mask.size() :', decoder_mask.size())                             # decoder_mask.size()               : torch.Size([8, 53])
        # print('grid_labels.size()  :', grid_labels.size())                              # grid_labels.size()                : torch.Size([8, 53, 44])
        ...
        # print(f'grid_labels : {grid_labels}')
        ...
        grid_state = grid_state.view(self.args.train_batch_size, -1)
        ...
        features_dropped = self.dropout(features_state)
        features_dropped = features_dropped.reshape(self.args.train_batch_size, grid_labels.size(1), grid_labels.size(2)).float()
        ...
        features_dropped = features_dropped.view(self.args.train_batch_size, -1, features_dropped.size(-1))
        grid_labels = grid_labels.view(self.args.train_batch_size, -1, features_dropped.size(-1))
...
def init_args():
    ...
    parser.add_argument('--train_mode', type=str, default='pseudo_labeling', choices=['train_quad', 'pseudo_labeling', 'train_scorer', 'do_filtering', 'train_quad_with_data_augmentation', 'do_reranking'])
    ...
    parser.add_argument("--model_name_or_path", type=str, default='') # "/ssd-data1/cxf2022/00.model_base/t5_base" if train else "../output/quad/model/acos/dataset=rest16,b='quad',seed=42"
    parser.add_argument("--accelerator", type=str, default='gpu')
    parser.add_argument("--seed", type=int, default=42)
    parser.add_argument("--devices", type=int, default=5)
    parser.add_argument("--dataset", type=str, default="acos/rest16", choices=['acos/rest16', 'acos/laptop16', 'asqp/rest16', 'asqp/rest15'])
    ...
    parser.add_argument("--self_training_data_dir", type=str, default='', choices=['', '../output/filter/acos/rest16.json'])
    ...
    if args.train_mode == 'train_quad':
        ...
        # Only set default model path if not already specified
        if args.model_name_or_path == '':
            args.model_name_or_path = "/data/cxf2022/dl_project/00.model_base/t5_large"
        if args.train_batch_size == 16:  # default value, set to smaller for t5_large
...
if __name__ == '__main__':
    args = init_args()
    # CUDA_VISIBLE_DEVICES is already set at the top of the file
    ...
    if args.use_sl:
        Model = LightningModule_SL(args)
    else:
        Model = LightningModule(args)
