...
class sl_model(nn.Module):
    def __init__(self, args, tokenizer):
        ...
        self.min_con_thre = args.min_con_thre
        self.avg_con_thre = args.avg_con_thre
        self.ac_loss_lambda = args.ac_loss_lambda
        self.sp_loss_lambda = args.sp_loss_lambda
        self.grid_loss_lambda = args.grid_loss_lambda
        self.cont_loss = args.cont_loss
        self.cont_temp = args.cont_temp
        self.use_sl = False

        self.model = T5ForConditionalGeneration.from_pretrained(args.model_name_or_path)

        # vague parameter
        self.dropout = nn.Dropout(self.vp_rate)  # 0.4
        self.vp_dropout_time = self.vp_dropout_time  # 5
        self.gama = args.gama
        self.m = args.m
    ...
    def forward(self,
        ...
        Seq2Seq_outputs = Seq2SeqLMOutput(loss=loss, logits=all_logits)

        return Seq2Seq_outputs, encoder_last_hidden_states, decoder_last_layer_hiddens


class T5FineTuner(pl.LightningModule):
    """ Fine tune a pre-trained T5 model """

    def __init__(self, tokenizer, sl_model, grid_learner, at_learner=None, ac_learner=None, ot_learner=None,
                 sp_learner=None):
        ...
        self.model = sl_model  # if args.use_sl : sl_model()   ;   else T5
        self.grid_learner = grid_learner
        self.at_learner = at_learner
        self.ac_learner = ac_learner
        self.ot_learner = ot_learner
        self.sp_learner = sp_learner
    ...
    def forward(self,
        ...
        if use_sl:
            main_pred, encoder_last_hidden_states, decoder_hidden_states = self.model(input_ids=input_ids,
                                                                                      attention_mask=attention_mask,
            ...
            main_pred = self.model(input_ids,
                                   attention_mask=attention_mask,
                                   decoder_input_ids=decoder_input_ids,
                                   decoder_attention_mask=decoder_attention_mask,
                                   labels=labels,
            ...
            encoder_last_hidden_states = main_pred.encoder_last_hidden_state
            decoder_hidden_states = main_pred.decoder_hidden_states

            # sentiment element contrastive loss
            at_pred = self.at_learner(encoder_last_hidden_states, attention_mask)
            ot_pred = self.ot_learner(encoder_last_hidden_states, attention_mask)
            sp_pred = self.sp_learner(encoder_last_hidden_states, attention_mask)

            # get final encoder layer representation
            masked_last_state = torch.mul(encoder_last_hidden_states, attention_mask.unsqueeze(-1))
            pooled_encoder_layer = torch.sum(masked_last_state, dim=1)
            pooled_encoder_layer = normalize(pooled_encoder_layer, p=2.0, dim=1)

        return main_pred, encoder_last_hidden_states, decoder_hidden_states, sp_pred, ot_pred, at_pred, pooled_encoder_layer
    ...
    def configure_optimizers(self):
        ...
        model = self.model
        grid_learner = self.grid_learner
        ...
        optimizer_grouped_parameters = [
            ...
            {
                "params": [p for n, p in grid_learner.named_parameters() if not any(nd in n for nd in no_decay)],
                "weight_decay": self.args.weight_decay,
            },
            {
                "params": [p for n, p in grid_learner.named_parameters() if any(nd in n for nd in no_decay)],
                "weight_decay": 0.0,
            },
        ]
    ...
    def get_tqdm_dict(self):
        tqdm_dict = {"loss": "{:.4f}".format(self.trainer.callback_metrics['avg_loss']),
